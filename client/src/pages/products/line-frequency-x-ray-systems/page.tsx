...
export default function LineFrequencyXRaySystemsPage() {
  const navigate = useNavigate();
  const { content, images, features, variants, loading, error } = useProduct(getProductApiSlug('line-frequency-x-ray-systems'));
  ...
  const mainImageUrl = mainImage?.image ? getImageUrl(mainImage.image) : getDefaultImageUrl('2025/05/Line-Frequency-New.jpg');
  const allImages = images.length > 0 ? images.map(img => img.image ? getImageUrl(img.image) : '').filter(Boolean) : [mainImageUrl];
  const productName = content?.title || 'Line Frequency X-Ray Systems';
  const productDescription = content?.description || 'Durable, reliable, and cost-effective medical imaging solutions for hospitals and diagnostic centers.';
  const seoConfig = getSEOConfig('/products/line-frequency-x-ray-systems');
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-12">
            <div className="mb-12 text-center" data-aos="fade-down">
              <h1 className="text-5xl font-medium text-[#7DC244]">
                {content?.title || 'Line Frequency X-Ray Systems'}
              </h1>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
              <div className="space-y-4" data-aos="fade-right">
                <ProductImageCarousel
                  images={allImages}
                  altText={mainImage?.image?.altText || content?.title || 'Line Frequency X-Ray Systems'}
                />
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-12">
            ...
            <div className="flex flex-wrap justify-center gap-8">
              {productCards.length > 0 ? (
                productCards
                  .filter(card => card.name !== 'Line Frequency X-Ray Systems')
                  .map((product, idx) => (
