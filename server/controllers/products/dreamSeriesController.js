const { DreamSeriesPageContent, DreamSeriesImage, DreamSeriesFeature, DreamSeriesVariant, DreamSeriesHospital, Media } = require('../../models');
...
exports.getHospitals = async (req, res) => {
  try {
    const hospitals = await DreamSeriesHospital.findAll({
      where: { isActive: true },
      order: [['orderIndex', 'ASC']],
    ...
    console.error('Get Dream Series Hospitals Error:', error);
    return status.errorResponse(res, error.message);
  }
};
...
exports.createHospital = async (req, res) => {
  try {
    const hospital = await DreamSeriesHospital.create(req.body);
    const created = await DreamSeriesHospital.findByPk(hospital.id, {
      include: [{ model: Media, as: 'hospitalLogo', required: false }]
    });
    return status.createdResponse(res, "Hospital created", created);
  } catch (error) {
    console.error('Create Dream Series Hospital Error:', error);
    return status.errorResponse(res, error.message);
  }
};
...
exports.updateHospital = async (req, res) => {
  try {
    const hospital = await DreamSeriesHospital.findByPk(req.params.id);
    if (!hospital) return status.notFoundResponse(res, "Hospital not found");
    await hospital.update(req.body);
    const updated = await DreamSeriesHospital.findByPk(req.params.id, {
      include: [{ model: Media, as: 'hospitalLogo', required: false }]
    });
    return status.successResponse(res, "Hospital updated", updated);
  } catch (error) {
    console.error('Update Dream Series Hospital Error:', error);
    return status.errorResponse(res, error.message);
  }
};
...
exports.deleteHospital = async (req, res) => {
  try {
    ...
    console.error('Delete Dream Series Hospital Error:', error);
    return status.errorResponse(res, error.message);
  }
};
