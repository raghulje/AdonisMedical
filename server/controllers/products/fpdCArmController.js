const { FpdCArmPageContent, FpdCArmImage, FpdCArmFeature, FpdCArmVariant, FpdCArmHospital, Media } = require('../../models');
...
exports.getHospitals = async (req, res) => {
  try {
    const hospitals = await FpdCArmHospital.findAll({
      where: { isActive: true },
      order: [['orderIndex', 'ASC']],
    ...
    console.error('Get FPD C-Arm Hospitals Error:', error);
    return status.errorResponse(res, error.message);
  }
};
...
exports.createHospital = async (req, res) => {
  try {
    const hospital = await FpdCArmHospital.create(req.body);
    const created = await FpdCArmHospital.findByPk(hospital.id, {
      include: [{ model: Media, as: 'hospitalLogo', required: false }]
    });
    return status.createdResponse(res, "Hospital created", created);
  } catch (error) {
    console.error('Create FPD C-Arm Hospital Error:', error);
    return status.errorResponse(res, error.message);
  }
};
...
exports.updateHospital = async (req, res) => {
  try {
    const hospital = await FpdCArmHospital.findByPk(req.params.id);
    if (!hospital) return status.notFoundResponse(res, "Hospital not found");
    await hospital.update(req.body);
    const updated = await FpdCArmHospital.findByPk(req.params.id, {
      include: [{ model: Media, as: 'hospitalLogo', required: false }]
    });
    return status.successResponse(res, "Hospital updated", updated);
  } catch (error) {
    console.error('Update FPD C-Arm Hospital Error:', error);
    return status.errorResponse(res, error.message);
  }
};
...
exports.deleteHospital = async (req, res) => {
  try {
    ...
    console.error('Delete FPD C-Arm Hospital Error:', error);
    return status.errorResponse(res, error.message);
  }
};
